#
# AdWinX-Sigmob 二进制版 podspec（由 Package/build.sh 自动渲染，勿手改）
...
Pod::Spec.new do |s|
  ...
  s.version          = '0.1.0'
  s.summary          = 'AdWinX adapter for Sigmob — 二进制分发。'

  s.description      = <<-DESC
AdWinX 聚合 SDK 的 Sigmob Adapter，覆盖开屏/激励视频/插屏广告类型。
本 pod 为二进制形态（xcframework），源码形态见仓库根目录 AdWinX-Sigmob.podspec。
                       DESC
  ...
  s.source           = { :http => 'https://github.com/mumabinggan/AdWinX/releases/download/AdWinX-Sigmob/0.1.0/AdWinX-Sigmob.xcframework.zip' }

  s.ios.deployment_target = '12.0'

  # 二进制：源码版 source_files 的替代
  s.vendored_frameworks = 'AdWinX-Sigmob.xcframework'
